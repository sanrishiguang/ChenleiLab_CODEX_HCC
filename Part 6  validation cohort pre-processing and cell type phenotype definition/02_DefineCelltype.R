...
ValidationData <- readRDS("~/Desktop/Project/Codex/Analysis_07v_Validation/01_GenerateSeuratInput/Outputs/ValidationData.Rds")
...
pdf("20220407_SFig1_ComBat_codex_F_merged.pdf", width = 4, height = 4)
p
dev.off()

p = DimPlot(codex_F, reduction = "umap", group.by = "TMA", split.by = "TMA", ncol = 5,
            order = rev(unique(codex_F@meta.data$TMA))) + NoLegend()
pdf("20220407_SFig1_ComBat_codex_F_split.pdf", width = 8, height = 4)
p
dev.off()
...
pdf("20220407_SFig1_ComBat_codex_F_com_merged.pdf", width = 4, height = 4)
p
dev.off()

p = DimPlot(codex_F_com, reduction = "umap", group.by = "TMA", split.by = "TMA", ncol = 5,
            order = rev(unique(codex_F_com@meta.data$TMA))) + NoLegend()
pdf("20220407_SFig1_ComBat_codex_F_com_split.pdf", width = 8, height = 4)
p
dev.off()
...
saveRDS(PhenoGraph_codexFcom_scale_result_k25, file = "PhenoGraph_codexFcom_scale_result_k25.Rds")
...
pdf("20220408_SFig1_Celltype_AllMarkers_heatmap_100.pdf", width = 12, height = 5)
p
dev.off()

p = DoHeatmap(subset(codex_F_com_phenograph.seurat, downsample=5000), slot = "scale.data", assay = "CODEX",
              features = rownames(codex_F_com_phenograph.seurat), angle = 90, size = 4, disp.min = -2, disp.max = 2) + NoLegend()
pdf("20220408_SFig1_Celltype_AllMarkers_heatmap_5000.pdf", width = 12, height = 5)
p
dev.off()

# Define cluster 29 as Unidentified
...
saveRDS(PhenoGraph_13cm_result_k25, file = "PhenoGraph_13cm_result_k25.Rds")

Idents(codex_F_com_rmNeg_13cm.seurat) = PhenoGraph_13cm_result_k25
levels(codex_F_com_rmNeg_13cm.seurat) = as.character(1:59)
...
pdf("20220408_SFig1_Celltype_CellMarkers_heatmap_100.pdf", width = 12, height = 3)
p
dev.off()

p = DoHeatmap(subset(codex_F_com_rmNeg_13cm.seurat, downsample=5000), slot = "scale.data", assay = "CODEX",
              features = rownames(codex_F_com_rmNeg_13cm.seurat), angle = 90, size = 4,disp.min = -2, disp.max = 2) + NoLegend()
pdf("20220408_SFig1_Celltype_CellMarkers_heatmap_5000.pdf", width = 12, height = 3)
p
dev.off()
...
new.cluster.ids <- c("Tumor cells",#1
                     ...
                     "Tumor cells",#9 
                     "CD8+ T cells",#10
                     "Fibroblasts/Endothelial cells",#11
                     "CD4+ T cells",#12
                     ...
                     "Tumor cells",#30
                     "Tumor cells",#31
                     "Lymphatic endothelial cells",#32  Lymphatic endothelial cells Podo
                     "Tumor cells",#33
                     "Tumor cells",#34
                     "Tumor cells",#35
                     "Tumor cells",#36
                     "Tumor cells",#37
                     "Tumor cells",#38
                     "Biliary tract cells",#39
                     "Tumor cells",#40
                     "Tumor cells",#41
                     "Tumor cells",#42
                     "Biliary tract cells",#43
                     "Biliary tract cells",#44
                     "Lymphatic endothelial cells",#45
                     "Tumor cells",#46
                     "Tumor cells",#47 
                     "Tumor cells",#48 
                     "Tumor cells",#49 
...
cols = c("#8358CF", # B cells
         ...
         "#27A7FA", # CD8+ T cells
         "#62FFFF", # Macrophages
         "#207720", # Fibroblasts/Endothelial cells
         "#74F882", # Fibroblasts
...
# Modify the UMAP of codex_F_com with defined cell types. Sample 1/200 for each cell type.
subset_list = list()
downsample_cycle = round(table(Idents(codex_F_com))/200,0)
for (i in 1:11) {
  temp = subset(codex_F_com, idents = names(downsample_cycle)[i])
  subset_list[[i]] = subset(temp, downsample = downsample_cycle[i])
}
codex_F_com_subset_sample_prob = merge(x = subset_list[[1]], y = unlist(subset_list)[2:11], merge.data = T) # no scale data
codex_F_com_subset_sample_prob = subset(codex_F_com, cells = colnames(codex_F_com_subset_sample_prob))

p = DimPlot(codex_F_com_subset_sample_prob, 
            reduction = "umap", 
            group.by = "celltype",
            label = FALSE, 
            repel = T,
...
pdf("20220408_Fig1_Celltype_Umap_Merged.pdf", width = 6, height = 8)
p
dev.off()
...
pdf("20220408_Fig1_Celltype_Umap_Split.pdf", width = 10, height = 8)
p
dev.off()

# Modify the HEATMAP of codex_F_com with defined cell types. Sample 7000 for each cell type.
temp_subset = subset(codex_F_com, features = CellMarkers_13)
...
ha = HeatmapAnnotation(celltype = annotation_col$celltype,
                       col = list(celltype = c("B cells" = "#8358CF",
                                               ...
                                               "CD8+ T cells" = "#27A7FA",
                                               "Macrophages" = "#62FFFF",
                                               "Fibroblasts/Endothelial cells" = "#207720",
                                               "Fibroblasts" = "#74F882",
...
pdf("20220408_Fig1_heatmap_7000.pdf", width = 10, height = 5)
ht
dev.off()

# Better heatmap can be obtained via rescaling genes.

# FeaturePlot to detect the expression of all features
p = FeaturePlot(codex_F_com_subset_sample_prob,
                slot = "scale.data",
...
pdf("20220408_Celltype_Umap_FeaturesExpres.pdf", width = 20, height = 17)
p
dev.off()

#### Delete Unidentified cells ####
# Second round QC, pick passed cells to proceed following analysis
UnidentifiedCellID = WhichCells(codex_F_com, 
                       idents = "Unidentified")
DeletedCellID = unique(UnidentifiedCellID)
...
pal<-colorRampPalette(cols)
image(x=1:10,y=1,z=as.matrix(1:10),col=pal(10))
...
codex_F_com_F_subset_sample_prob = merge(x = subset_list[[1]], y = unlist(subset_list)[2:10], merge.data = T) # no scale data
...
p = DimPlot(codex_F_com_F_subset_sample_prob, 
        ...
        order = rev(orders),
        pt.size = 0.4) +
  theme(legend.position = "top") + NoLegend()
  guides(color = guide_legend(ncol = 2, byrow = T))

pdf("20220408_Fig1_Celltype_Umap_Merged.pdf", width = 5, height = 5)
p
dev.off()
...
pdf("20220408_Fig1_Celltype_Umap_Split.pdf", width = 13, height = 8)
p
dev.off()

# Modify the HEATMAP of codex_F_com_F with defined cell types. Sample 7000 for each cell type.
...
p = ggplot(data = Freq, aes(x = Celltype, y = Freq, fill = Celltype)) + 
  ...
  scale_fill_manual(values = c("#8358CF", "#AE2519", "#D083FF", "#27A7FA", "#F6D026", "#74F882", "#207720", "#EE6600", "#62FFFF", "#FA339A")) +
...
pdf("20220408_Fig1_heatmap_barplot.pdf", width = 10, height = 3)
p
dev.off()
...
p = ggplot(data = Freq, aes(x = Celltype, y = Freq, fill = Celltype)) + 
  ...
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  NoLegend() + scale_y_break(c(10,60)) + 
  geom_text(aes(label = paste0(Freq,"%")), vjust = -.5) + 
  ylim(0,70)

pdf("20220408_Fig1_heatmap_barplot_NoLegend.pdf", width = 7, height = 3)
p
dev.off()
