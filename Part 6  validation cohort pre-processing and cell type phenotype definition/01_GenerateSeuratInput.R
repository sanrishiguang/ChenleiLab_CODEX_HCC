...
#### Extract TMA and reg info of the validation data ####
load("~/Desktop/Project/Codex/Analysis_07v_Validation/01_GenerateSeuratInput/HCC401.Rdata")
ValidationData.df = df_validation[order(df_validation$TMA),]
TMA_reg_info = list()
for (i in 1:length(unique(ValidationData.df$TMA))) {
  TMA_reg_info[[i]] = paste0("reg0",sprintf("%02d",ValidationData.df[which(ValidationData.df$TMA==i+1),]$Reg),"_qupath_cell_seg_data.txt")
}
...
saveRDS(csd,file="csd.Rds")
saveRDS(csd_raw,file="csd_raw.Rds")
saveRDS(ValidationData.df, file = "ValidationData.Rds")
...
plot(sort(FilterID$rowsum)[1074898:1124898])
abline(h = 200000, col="red")
dev.off()

FilterID = subset(FilterID, rowsum > 700 & rowsum < 200000) 

limit_Exprs = c(5000,40000,20000,800,50000,15000,
                25000,10000,20000,10000,10000,20000,
                40000,10000,30000,40000,40000,25000,
                40000,7000,45000,15000,max(FilterID$HistoneH3),35000,25000,max(FilterID$KI67),
                2000,10000,15000,6000,30000,15000,
                10000,max(FilterID$Podoplanin),4000,35000)
for (i in 1:36) {
  pdf(paste0(colnames(FilterID)[i],".pdf"),width = 4,height = 4)
  plot(sort(FilterID[[i]])[1074898:1124898],ylab=paste0(colnames(FilterID)[i],"_[1074898:1124898]"))
  abline(h = limit_Exprs[i], col="red")
  dev.off()
}

FilterID = subset(FilterID, actin < 5000 & aSMA < 40000 & c_Myc < 20000 & Caspase_3 < 800 & CD107A < 50000 & CD11C < 15000 &
                    CD163 < 25000 & CD20 < 10000 & CD21 < 20000 & CD3 < 10000 & CD31 < 10000 & CD4 < 20000 &
                    CD44 < 40000 & CD45 < 10000 & CD45RO < 30000 & CD68 < 40000 & CD8 < 40000 & E_Cadherin < 25000 &
                    FOXP3 < 40000 & Glypican3 < 7000 & Hepar1 < 45000 & HIF1a < 15000 & HLA_DR < 35000 & Keratin < 25000 &
                    p_AMPK < 2000 & p_mTOR < 10000 & p_S6 < 15000 & p53 < 6000 & Pan_CK < 30000 & PD_L1 < 15000 &
                    PD1 < 10000 & Twist1 < 4000 & Vimentin < 35000)

FilterID = FilterID$Cell.ID

rm(DataFilter)
saveRDS(FilterID,file = "FilterID.Rds")
