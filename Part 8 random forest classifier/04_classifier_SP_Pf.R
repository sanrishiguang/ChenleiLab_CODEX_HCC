...
####Train
df.train <- Tra_mydata_df.train[c(com_ct,com_subtypes,com_spaInt_filter, 'SP_Pf')]

df.train <- df.train %>% rename(class = SP_Pf)
...
df.validate <- Tra_mydata_df.validate[c(com_ct,com_subtypes,com_spaInt_filter, 'SP_Pf')]

df.validate <- df.validate %>% rename(class = SP_Pf)
...
##repeat
df.train.sig_fil2 <- df.train.sig_fil3
...
df_imp <- as.data.frame(importance (fit.forest, type=2))  ##GINI系数
...
write.csv(df_imp,file='./SP_Pf_GINI.csv')
...
Val_mydata_anno$SP_Pf <- forest.pred
table(Val_mydata_anno$SP_Pf)
...
library(survival)
library(survminer)
fit <- survfit(Surv(RFSday, RFS01) ~ SP_Pf, data = Val_mydata_anno)
ggsurvplot(fit,palette = "npg",
           risk.table = TRUE, pval = TRUE,
           conf.int = FALSE, xlab="Time in Days",
           ggtheme = theme_classic())

save(Val_SP_Pf_predict,Val_mydata, fit.forest, Tra_mydata_df.train,df.train, df.validate, Tra_mydata_df.validate, file = 'SP_Pf.Rdata')
saveRDS(Val_SP_Pf_predict, file='Validation _predict_SP_Pf.Rds')
save(list=ls(), file = "SP_Pf_all.Rdata")
