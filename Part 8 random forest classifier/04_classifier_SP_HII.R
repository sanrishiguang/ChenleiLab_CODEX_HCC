...
####Train
df.train <- Tra_mydata_df.train[c(com_ct,com_subtypes,com_spaInt_filter, 'SP_HII')]

df.train <- df.train %>% rename(class = SP_HII)
...
df.validate <- Tra_mydata_df.validate[c(com_ct,com_subtypes,com_spaInt_filter, 'SP_HII')]

df.validate <- df.validate %>% rename(class = SP_HII)
...
write.csv(df_imp,file='./SP_HII_GINI.csv')
...
Val_mydata_anno$SP_HII <- forest.pred
table(Val_mydata_anno$SP_HII)
...
library(survival)
library(survminer)
fit <- survfit(Surv(RFSday, RFS01) ~ SP_HII, data = Val_mydata_anno)
ggsurvplot(fit,palette = "npg",
           risk.table = TRUE, pval = TRUE,
           conf.int = FALSE, xlab="Time in Days",
           ggtheme = theme_classic())

save(Val_SP_HII_predict,Val_mydata, fit.forest, Tra_mydata_df.train,df.train, df.validate, Tra_mydata_df.validate, file = 'SP_HII.Rdata')
saveRDS(Val_SP_HII_predict, file='Validation _predict_SP_HII.Rds')
save(list=ls(), file = "SP_HII_all.Rdata")
