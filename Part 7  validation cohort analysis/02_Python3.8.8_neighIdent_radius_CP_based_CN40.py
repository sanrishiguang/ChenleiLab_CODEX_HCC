...
radius = 50
path_to_data = r'E:\11. CODEX\ValidationData\testing_allReg_seurat_._analysis_20220412_AllSubtypes_celltype.csv'
X = 'XMin'
Y = 'YMin'
reg = 'Class'
file_type = 'csv'

cluster_col = 'Allsubtypes' #CODEX Clusters Nucleus Intensity
...
windows2 = pd.concat(tissues[0:100])
...
for i in range(15, 55, 5):
    ...
    cells['neighborhood10'] = labelskm
    cells[neighborhood_name] = cells[neighborhood_name].astype('category')
    cells.to_csv("E:\\myPythonProject\\ValidationData\\20220412 AllsubtypeCN\\cells_r=50_CN="+str(i)+".csv")
    k_to_plot = 10
    niche_clusters = (k_centroids[k_to_plot])
    tissue_avgs = values.mean(axis = 0)
    fc = np.log2(((niche_clusters+tissue_avgs)/(niche_clusters+tissue_avgs).sum(axis = 1, keepdims = True))/tissue_avgs)
    fc = pd.DataFrame(fc,columns = sum_cols)
    s=sns.clustermap(fc, vmin =-3,vmax = 3,cmap = 'bwr',row_cluster = False, figsize=(20,10))
    s.savefig("E:\\myPythonProject\\ValidationData\\20220412 AllsubtypeCN\\r=50_CN="+str(i)+".pdf")
    s1=sns.clustermap(fc, vmin =-3,vmax = 3,cmap = 'bwr',row_cluster = True, figsize=(20,15))
    s1.savefig("E:\\myPythonProject\\ValidationData\\20220412 AllsubtypeCN\\r=50_CN="+str(i)+"clustering.pdf")
