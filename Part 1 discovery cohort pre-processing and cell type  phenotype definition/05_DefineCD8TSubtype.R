...
levels(codex_F_com_F_CD8T) = as.character(1:34)
...
pdf("20220310_Subtype_CD8T_Heatmap_100.pdf", width = 12, height = 3)
p
dev.off()

p = DoHeatmap(subset(codex_F_com_F_CD8T_SelectedFeatures, downsample=5000), slot = "scale.data", assay = "CODEX",
              features = rownames(codex_F_com_F_CD8T_SelectedFeatures), angle = 90, size = 4,disp.min = -2, disp.max = 2) + NoLegend()
pdf("20220310_Subtype_CD8T_Heatmap_5000.pdf", width = 12, height = 3)
p
dev.off()
...
pdf("20220310_Subtype_CD8T_Umap_Split.pdf", width = 12, height = 6)
p
dev.off()
...
# Definition
new.cluster.ids <- c("CD8T_HLA-DR+",#1
                     "CD8T_p-S6+",#2 
                     "CD8T_p-S6+",#3
                     "CD8T_others",#4
                     "CD8T_c-Myc+",#5
                     ...
                     "CD8T_CD44+",#8 
                     "CD8T_PD1+",#9 
                     "CD8T_p-S6+_CD45RO+",#10
                     "CD8T_CD45RO+_CD44+",#11
                     "CD8T_CD45RO+_int",#12
                     "CD8T_CD107a+_int",#13 
                     "CD8T_HLA-DR+_int",#14
                     "CD8T_p-S6+_int",#15
                     "CD8T_others",#16
                     "CD8T_c-Myc+_p-S6+",#17
                     "CD8T_c-Myc+_CD45RO+",#18
                     "CD8T_c-Myc+_CD107a+",#19
                     "CD8T_KI67+",#20
                     "CD8T_others",#21
                     "CD8T_c-Myc+",#22
                     "CD8T_CD107a+",#23
...
#### V2: subtype: CD8T: modification on V1 after checking back on MAV ####
load("~/Desktop/Project/Codex/Analysis_07v/06_DefineCD8TSubtype/Outputs_v1/07_DefineCD8TSubtype_V1.RData")

## k=20: 35 clusters
PhenoGraph_CD8T_result_k20 <-as.numeric(membership(Rphenograph(t(codex_F_com_F_CD8T_phenograph),k=20)))

Idents(codex_F_com_F_CD8T) = PhenoGraph_CD8T_result_k20
levels(codex_F_com_F_CD8T) = as.character(1:35)
...
## k=15: 40 clusters
PhenoGraph_CD8T_result_k15 <-as.numeric(membership(Rphenograph(t(codex_F_com_F_CD8T_phenograph),k=15)))

Idents(codex_F_com_F_CD8T) = PhenoGraph_CD8T_result_k15
levels(codex_F_com_F_CD8T) = as.character(1:40)
...
## k=15 is fine.
save(PhenoGraph_CD8T_result_k15, file = "PhenoGraph_CD8T_result_k15.RData")
save(codex_F_com_F_CD8T,file = "codex_F_com_F_CD8T_PhenoGraph_PCA_UMAP.RData")
...
p = DoHeatmap(subset(codex_F_com_F_CD8T_SelectedFeatures, downsample=100), slot = "scale.data", assay = "CODEX",
              features = rownames(codex_F_com_F_CD8T_SelectedFeatures), angle = 90, size = 4,disp.min = -2, disp.max = 2) + NoLegend()
pdf("20220311_Subtype_CD8T_Heatmap_100.pdf", width = 12, height = 3)
p
dev.off()

p = DoHeatmap(subset(codex_F_com_F_CD8T_SelectedFeatures, downsample=5000), slot = "scale.data", assay = "CODEX",
              features = rownames(codex_F_com_F_CD8T_SelectedFeatures), angle = 90, size = 4,disp.min = -2, disp.max = 2) + NoLegend()
pdf("20220311_Subtype_CD8T_Heatmap_5000.pdf", width = 12, height = 3)
p
dev.off()

# UMAP
codex_F_com_F_CD8T@meta.data$CD8T_phenograph = PhenoGraph_CD8T_result_k15
p = DimPlot(subset(codex_F_com_F_CD8T,downsample=500), 
            reduction = "umap", 
            label = FALSE, 
            repel = T,
...
pdf("20220311_Subtype_CD8T_Umap_Split.pdf", width = 12, height = 6)
p
dev.off()

saveRDS(test_php, file = "TrainingV2_CD8Tsub_test_php.Rds")
...
new.cluster.ids <- c("CD8T_HLA-DR+",#1
                     ...
                     "CD8T_c-Myc+",#25
                     "CD8T_KI67+",#26
                     "CD8T_p-S6+_CD45RO+",#27
                     "CD8T_CD107a+_c-Myc+",#28
                     "CD8T_CD107a+",#29
                     "CD8T_HIF1a+",#30
                     "CD8T_KI67+",#31
                     "CD8T_CD44+_int",#32
...
#name the cell seg file. You need one cell seg file per region. For region 1 the cell seg file should be name reg001_
write.table(gc_csd_final4, file = "testing_allReg_seurat_._analysis_20220311_CD8TFeatures_CD8TSubtype_V2.csv", sep=",", col.names = NA, row.names = TRUE)

#### V3: subtype: CD8T: modification on V2 after checking back on MAV ####
load("~/Desktop/Project/Codex/Analysis_07v/06_DefineCD8TSubtype/Outputs_v2/07_DefineCD8TSubtype_V2.RData")

Idents(codex_F_com_F_CD8T)= codex_F_com_F_CD8T@meta.data$CD8T_phenograph
...
#name the cell seg file. You need one cell seg file per region. For region 1 the cell seg file should be name reg001_
write.table(gc_csd_final4, file = "testing_allReg_seurat_._analysis_20220314_CD8TFeatures_CD8TSubtype_V3.csv", sep=",", col.names = NA, row.names = TRUE)

CD8TSubtype = gc_csd_final4
save(CD8TSubtype,file = "20220315_CD8TSubtype.RData")

#### Subtype: UMAP and Heatmap ####
codex_F_com_F_CD8T <- subset(codex_F_com_F_CD8T,idents=c("CD8T_artifact"),invert = TRUE) %>%
  Seurat::NormalizeData(verbose = FALSE,normalization.method = NULL) %>%
  ScaleData(verbose = FALSE)
...
codex_F_com_F_subset_sample_prob = merge(x = subset_list[[1]], y = unlist(subset_list)[2:15], merge.data = T) # no scale data
...
image(x=1:15,y=1,z=as.matrix(1:15),col=pal(15))
...
pdf("20220418_CD8TSubtype_Umap_Merged_1in20.pdf", width = 5, height = 5)
p
dev.off()
...
pdf("20220418_heatmap_barplot_NoLegend.pdf", width = 15, height = 5)
p
dev.off()
...
ha = HeatmapAnnotation(CD8TSubtype = annotation_col$CD8TSubtype,
                       col = list(CD8TSubtype = c("CD8T_HLA-DR+" = "#FA339A",
                                                  "CD8T_p-S6+" = "#F6D026",
                                                  "CD8T_others" = "#74F882",
                                                  "CD8T_c-Myc+" = "#7E8EE3",
                                                  "CD8T_CD45RO+" = "#8358CF",
                                                  "CD8T_CD44+" = "#27A7FA",
                                                  "CD8T_CD107a+" = "#EE6600",
                                                  "CD8T_p-S6+_c-Myc+" = "#D083FF",
                                                  "CD8T_p-S6+_CD44+" = "#62FFFF",
                                                  "CD8T_KI67+" = "#AE2519",
                                                  "CD8T_p-S6+_CD45RO+" = "#9C9B99",
                                                  "CD8T_CD107a+_c-Myc+" = "#162C9B",
                                                  "CD8T_HIF1a+" = "#FBF88B",
                                                  "CD8T_CD44+_c-Myc+" = "#B58794",
                                                  "CD8T_KI67+_p-S6+" = "#149684")),
                       show_legend = F)
